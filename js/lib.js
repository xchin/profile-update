/*! vxpattern-lib-paypal | Version: 2.0.4 | Built from github.paypal.com/ConsumerWeb-UI/vxpattern-lib-paypal */
...
(function (win, doc, undefined) {
	...
	root.addEventListener('click', function (event) {
		 ...
		 */
		if (event.target.className.indexOf('vx_modal-trigger') !== -1) {
			modalId = event.target.getAttribute('data-modal-id');
			openVxModal(modalId);
		}
		if (event.target.className.indexOf('vx_modal-dismiss_trigger') !== -1 ||
			event.target.className.indexOf('vx_modal-dismiss_x-trigger') !== -1) {
			modalId = event.target.getAttribute('data-modal-id');
	...
	root.addEventListener('keyup', function (event) {
		if (event.keyCode === 27 && doc.body.className.indexOf('vx_hasOpenModal')) {
			closeVxModal();
		}
	});
...
(function (win, doc, undefined) {
	...
	function bindFlowListener() {
		root.addEventListener('click', function (event) {
			var target = event.target;
			if (target.className.indexOf('vx_flowTrigger') !== -1) {
				startFlow();
			}
		});
	}

	if (root.className.indexOf('vx_hasFadeTransition') !== -1) {
		bindFlowListener();
		setTimeout(function () {
			endFlow();
		}, 35);
